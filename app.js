class MusicStore {
    ...
    displayMusic() {
        ...
        // Apply sorting
        filteredMusic = this.sortMusic(filteredMusic);
    ...
    sortMusic(music) {
        switch(this.currentSort) {
            ...
            case 'artist':
                return music.sort((a, b) => a.artist.localeCompare(b.artist));
            case 'price-low':
                return music.sort((a, b) => a.price - b.price);
            case 'price-high':
                return music.sort((a, b) => b.price - a.price);
            default:
                return music;
        }
    }
    ...
    displayRecentlyPlayed() {
        ...
        recentTracks.innerHTML = this.recentlyPlayed.slice(-6).reverse().map(song => `
            <div class="recent-track" onclick="musicStore.previewTrack(${song.id})">
                <img src="${song.albumArt}" alt="${song.title}">
                <h5>${song.title}</h5>
                <p>${song.artist}</p>
            </div>
